...
void PrintDebug(const std::string &StrToPrint, std::ofstream &outfile, const int Dlevel)
{
...
//TODO commments
void GlobalPrintDebug(std::string StrToPrint, const int Dlevel)
{
    PrintDebug(StrToPrint, GLOBAL_debuglog, Dlevel);
}

void printsimplex(const simplex &blob, std::ostream& SSt)
{
   const time_t Ctime = time(0);
...
*/
int gcalc(double &grFinal, double &guFinal, WORKER_status &WStatus, 
          std::ofstream &outfile, unsigned long int cnt, double prev_g, double prev_u)
{
...
int gr_calc(std::string eStr, std::string sStr, double &gr, double &gu)
{
...
bool is_file_useful(std::ostringstream &FileName)
{
   std::fstream DatFile;
...
int StatChk(const pid_t pid, WORKER_status &Wstatus, const int Rank)
{
    ...
    {   std::ostringstream astring;
	//child process stopped with status of Ret
...
void PrintWsit(const WRK_situation Wsit, std::ofstream &outfile)
{
    if(Wsit == 0)
    {
...
MASTER_comand MASwork(const WORKER_status WSTAT, const int CPN, const simplex blob)
{
   ...
   if(WSTAT == undef && IsCPN_aux == false){
...
void updateMWstat(const WORKER_status WST, const int CPN, simplex &blob)
{
...
void updateVALUEs(const int CPN, const double gVALUE, const double gUNC, simplex &blob)
{
   for(int i = 0; i < blob.vertices; i++){
      if(blob.vertex[i].ProcID == CPN){
...
void SetVec(arma::vec &Tvec, const int dim, const int CPN, const simplex blob)
{
   for(int i = 0; i < blob.vertices; i++ ){
      if(blob.vertex[i].ProcID == CPN){
         for(int j = 0; j < dim; j++){
...
bool ChkErr(const simplex blob, bool III)
{
   bool FAIL = true;
...
MASTER_comand GetMcomm(const int CPN, const simplex blob)
{
   for(int i = 0; i < blob.vertices; i++){
      if(blob.vertex[i].ProcID == CPN){
...
WORKER_status getPrevWstat(const simplex blob, int CPN){
...
bool DetectChangeWstat(const simplex blob, const WORKER_status PWstat, int CPN)
{
   for(int i = 0; i < blob.vertices; i++){
...
int DoScript(std::string scriptStr){
   pid_t SCPTpid;
//   pid_t SCPTparent;
   int SCPTstatus;
