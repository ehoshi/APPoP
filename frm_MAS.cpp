...
void MASTER(int id, int P, arma::mat inP)
{
   int PointID_count = 0;
...
//Receive the status from initialization
   for(int i = 0; i < blob.vertices; i++){
      ...
      MPI_Recv(&Wtemp, 1, MPI_INT, i+1, MPI_ANY_TAG, 
               MPI_COMM_WORLD, &StatMPI);
   ...
   for(int Pj = 1; Pj < P; Pj++){
	 ...
	 if(Mcomm == GMTV)
	 {
		...
		PrintDebug(Mstring.str(), MASlog, 7);
         ...
         if(Mcomm == reinitiate || Mcomm == start)
	 {
	     {  std::ostringstream Mstring;
